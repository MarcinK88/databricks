...
-- MAGIC 
-- MAGIC Now let's compare behaviour when we drop and recreate these tables. Before we do that, let's use **SHOW CREATE TABLE** to capture the command needed to recreate **silver_managed** after we drop it. Uncomment and run the following cells.
